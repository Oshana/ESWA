...
class Classifier_MCNN:
    ...
    def train(self, x_train, y_train, x_test, y_test,y_true, x_val, y_val, pool_factor=None, 
              filter_size=None,do_train=True):
        window_size = 0.2
        n_train_batch = 10
        n_epochs = 200
...
#         x_train,y_train,x_val,y_val = self.split_train(x_train,y_train)
    ...
    def build_model(self, input_shapes, nb_classes, pool_factor, kernel_size):
        ...
        max_length = max([input_shape[0] for input_shape in input_shapes])
...
#             pool_size = int(int(conv_layer.shape[1])/pool_factor)
            pool_size = max(1, int(max_length / pool_factor))

            max_layer = keras.layers.MaxPooling1D(pool_size=pool_size, padding='same')(conv_layer)
        
            if max_layer.shape[1] != max_length:
                max_layer = keras.layers.Cropping1D(cropping=(0, max_length - int(max_layer.shape[1])))(max_layer)
...
#         pool_size = int(int(full_conv.shape[1])/pool_factor)
        pool_size = max(1, int(int(full_conv.shape[1]) / pool_factor))

        full_max = keras.layers.MaxPooling1D(pool_size=pool_size, padding='same')(full_conv)
    ...
    def fit(self, x_train, y_train, x_test, y_test,y_true, x_val, y_val):
        ...
        for pool_factor in self.pool_factors:
            for filter_size in self.filter_sizes:
                ...
                df_metrics, model , valid_loss, y_pred = self.train(x_train, y_train, x_test, 
                                                          y_test, y_true, x_val, y_val, pool_factor,filter_size)
    ...
    def predict(self, x_test, y_true,x_train,y_train,y_test, x_val, y_val):
        df_metrics, _ , _, y_pred = self.train(x_train, y_train, x_test, y_test,y_true, x_val, y_val, do_train=False)
