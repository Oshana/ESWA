...
#     problem = config['data_dir'].split('/')[-1]
    problem = config['data_dir'].split('/')[-2]
...
#             train_file = config['data_dir'] + '/' + problem + "_TRAIN.csv"
#             test_file = config['data_dir'] + '/' + problem + "_TEST.csv"
            file_path = 'Dataset/Segmentation/ActivityRecognition/ActivityRecognition.csv'
            train_data, test_data = data_loader.load_segmentation_data(file_path, "Clean")
...
#         np.save(config['data_dir'] + "/" + problem, Data, allow_pickle=True)
        np.save('Dataset/Segmentation/ActivityRecognition/', Data, allow_pickle=True)
