...
def main(args):
    ...
    for problem in os.listdir(config['data_path']):  # for loop on the all datasets in "data_dir" directory
        ...
        best_test_evaluator = SupervisedTrainer(best_model, test_loader, device, config['loss_module'],
                                                print_interval=config['print_interval'], console=config['console'],
                                                print_conf_mat=True)
        best_aggr_metrics_test, all_metrics, predictions, targets, class_names = best_test_evaluator.evaluate(keep_all=True)
        ...
        class_wise_metrics, overall_report, additional_metrics, cm = evaluate_model(targets, predictions, class_labels)
        ...
        print(config['output_dir'] + '/holsteinEvals_all_results.pickle')

        with open(config['output_dir'] + '/holsteinEvals_all_results.pickle', 'wb') as handle:
...
if __name__ == '__main__':
    ...
    # -------------------------------------------- Input and Output --------------------------------------------------------
    parser.add_argument('--data_path', default='Dataset/Segmentation/', choices={'Dataset/UEA/', 'Dataset/Segmentation/', 'Dataset/ActBeCalf/'},
                        help='Data path')
